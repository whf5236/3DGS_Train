...
import { Popup, type ShowOptions } from './popup';
...
class EditorUI {
    ...
    constructor(events: Events, rootElementId: string = 'app-container') {
        localizeInit();
        ...
        // 获取自定义根元素或使用默认的 appContainer
        const rootElement = document.getElementById(rootElementId);
        const targetParent = rootElement || document.body;
        ...
        canvas.id = 'canvas';
        // set initial canvas size to prevent 0x0 render target error
        canvas.width = 800;
        canvas.height = 600;
        ...
        ['pointerdown', 'pointerup', 'pointermove', 'wheel', 'dblclick'].forEach((eventName) => {
            cursorLabel.dom.addEventListener(
                eventName, 
        ...
        targetParent.appendChild(appContainer.dom);
