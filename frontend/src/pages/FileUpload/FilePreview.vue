    ...
    <div v-else class="preview-container">
      ...
      <div v-else-if="isVideo && activeTab === 'video'" class="video-preview">
        ...
        <el-descriptions 
          ...
          :column="2" 
          border 
          class="video-info glass-descriptions"
          title="视频信息"
        >
          <el-descriptions-item label="分辨率">{{ videoInfo.width }}x{{ videoInfo.height }}</el-descriptions-item>
      ...
      <div v-else-if="isVideo && activeTab === 'frames'" class="frames-preview">
        ...
        <div v-else class="frames-content">
          ...
          <el-descriptions 
            ...
            :column="2" 
            border 
            class="extraction-info glass-descriptions"
            title="提取信息"
          >
            <el-descriptions-item label="原始帧率">{{ extractionInfo.original_fps }} fps</el-descriptions-item>
            <el-descriptions-item label="目标帧率">{{ extractionInfo.target_fps }} fps</el-descriptions-item>
            <el-descriptions-item label="总视频帧数">{{ extractionInfo.total_video_frames }}</el-descriptions-item>
            <el-descriptions-item label="提取帧数">{{ extractionInfo.extracted_frames }}</el-descriptions-item>
            <el-descriptions-item label="视频时长">{{ extractionInfo.duration_seconds }}s</el-descriptions-item>
            <el-descriptions-item label="提取时间">{{ formatDateTime(extractionInfo.extraction_timestamp) }}</el-descriptions-item>
          </el-descriptions>
        </div>
      </div>
...
const loadVideoInfo = async () => {
  ...
  try {
    ...
    const response = await api.get(`/video_info/${props.selectedFile.filename}`, {
      params: { username }
    })
...
const loadFrames = async () => {
  ...
  try {
    ...
    const username = userStore.user?.username || 'current_user'
    const response = await api.get(`/video_frames/${username}/${folderName}`)
...
const getFrameImageUrl = (framePath: string): string => {
  return `${api.defaults.baseURL}/frame_image/${framePath}`
}
...
.loading-frames, .frame-error, .no-frames {
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
