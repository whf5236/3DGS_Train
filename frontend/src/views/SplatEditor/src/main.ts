...
const main = async (rootElementId: string = 'splat-editor-root') => {
    // root events object
    ...
    // editor ui
    const editorUI = new EditorUI(events, rootElementId);
    ...
    // 保存到 window 对象，供外部访问
    window.scene = scene;
    (window.scene as any).events = events;
    ...
    return { scene, events, graphicsDevice };
};
