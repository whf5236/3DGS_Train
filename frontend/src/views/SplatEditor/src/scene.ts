...
class Scene {
            ...
            if (entries.length > 0) {
                ...
                if (entry) {
                    ...
                    if (entry.devicePixelContentBoxSize) {
                        ...
                        newWidth = Math.ceil(entry.contentBoxSize[0]!.inlineSize * pixelRatio);
                        newHeight = Math.ceil(entry.contentBoxSize[0]!.blockSize * pixelRatio);
                    }
                    
                    // only set canvas resize if dimensions are valid
    ...
    private onPreRender() {
        if (this.canvasResize) {
            ...
            if (this.canvasResize.width > 0 && this.canvasResize.height > 0) {
