...
const submitUpload = async () => {
  ...
  try {
    ...
    // 根据点云类型设置上传阶段
    let stage = 'colmap' // 默认阶段
    if (pointcloudType.value === 'gaussian') {
      stage = 'gaussian'
    } else if (pointcloudType.value === 'colmap') {
      stage = 'colmap'
    } else {
      // 其他类型（standard, lidar, mesh）也使用colmap阶段
      stage = 'colmap'
    }
    
    form.append('stage', stage)
    form.append('upload_type', 'pointcloud')
